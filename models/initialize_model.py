...
class MTL_Model(object):
    def __init__(self, shared_layers, specific_layers, learning_rate, lr_decay, lr_decay_epoch, momentum, weight_decay, args):
    ...
    #   construct the parameter
        ...
        # 设置输出的loss为每个样本的loss ，mean则输出为batch的平均loss
    ...
    def optimize_model(self, input_batch, label_batch, obj):
        ...
        batch_loss = self.criterion(output_batch, label_batch.to(torch.long))
        batch_loss_mean = batch_loss.mean()
        # 为了节省时间，使用mean的loss进行反向传播
        batch_loss_mean.backward()
        
        
        # 以下只有Central DP 使用，即每个梯度都进行加噪
        # TODO: 梯度裁剪   ****************
        # if obj.using_dp:
        #     print("check for clipping")
            # obj.DP.clip_gradients(model=self.shared_layers)
        # 根据计算的梯度使用优化器来更新模型的参数，沿着梯度的反方向更新参数
        # ********************************
        self.optimizer.step()
        ...
        #     obj.DP.add_noise(model=self.shared_layers)
        # ********************************
        # self.optimizer_state_dict = self.optimizer.state_dict()
        return batch_loss
    ...
    if args.mtl_model:
        ...
        if args.dataset == 'cifar10':
            ...
            if  args.model == 'resnet18':
                shared_layers = ResNet18(num_classes=100)
                specific_layers = None
            else:
